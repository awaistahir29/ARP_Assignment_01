...
void sigusr1_handler(int sig)
{
    ...
    check(write(fd_motorX, buffer, strlen(buffer) + 1));
...
int main(int argc, char *argv[])
{
    ...
    float random_error;
    float movement;
    fd_command = check(open(fifo_command_motorX, O_RDONLY));
    fd_inspection = check(open(fifo_inspection_motorX, O_RDONLY));
    ...
    int fd_motX_pid_i = check(open(fifo_motX_pid_inspection, O_WRONLY));
    sprintf(buffer, "%d", (int)getpid());
    check(write(fd_motX_pid_i, buffer, SIZE));
    ...
    fd_motorX = check(open(fifo_motorX_value, O_WRONLY));
    ...
    check(close(fd_motorX));
    unlink(fifo_motorX_value);
    fprintf(logfile, "files closed and unlinked\n");
